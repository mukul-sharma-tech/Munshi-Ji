...
import {
  ...
  Home, BarChart2, Heart, ArrowRight, Sparkles, AlertTriangle,
} from "lucide-react";
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
        {stats.map((s, i) => {
          ...
          return (
            <div
              key={s.label}
              className={`rounded-xl border ${a.border} bg-card p-4 sm:p-5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
              style={{ transitionDelay: `${i * 60}ms` }}
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-muted-foreground">{s.label}</span>
                <div className={`p-1.5 rounded-lg ${a.iconBg}`}>
                  <s.icon className={`h-4 w-4 ${a.iconColor}`} />
                </div>
              </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="h-1 w-full rounded-full bg-muted overflow-hidden">
                    <div
                      ...
                      style={{ width: mounted ? ins.barWidth : "0%", transitionDelay: `${i * 100 + 400}ms` }}
                    />
                  </div>
                </div>
              </div>
