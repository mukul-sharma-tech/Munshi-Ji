...
function RevealCard({ children, delay = 0, className = "" }: { children: React.ReactNode; delay?: number; className?: string }) {
  ...
  return (
    <div
      ref={ref}
      className={`transition-all duration-700 ease-out ${visible ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-95"} ${className}`}
    >
      {children}
    </div>
  );
    ...
    <div className="min-h-screen bg-[#04040f] text-white overflow-x-hidden" style={{ fontFamily: "var(--font-inter, sans-serif)" }}>
      ...
      <nav className="relative z-20 flex items-center justify-between px-6 sm:px-14 py-5 border-b border-white/5 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/dashboard" className="text-xs font-semibold bg-emerald-500 hover:bg-emerald-400 text-black px-4 py-1.5 rounded-lg shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 transition-all hover:scale-105">Get Started</Link>
        </div>
      </nav>
      ...
      <section className="relative z-10 flex flex-col items-center text-center px-6 pt-24 pb-16 sm:pt-36 sm:pb-24">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 mt-10 animate-[fadeInUp_0.7s_ease_0.4s_forwards] opacity-0">
          ...
          <Link href="/dashboard" className="flex items-center gap-2 text-sm text-white/60 hover:text-white border border-white/10 hover:border-white/25 bg-white/3 hover:bg-white/8 px-7 py-3 rounded-xl backdrop-blur-sm transition-all hover:scale-105">
            Explore Dashboard
          </Link>
        </div>
      ...
      <div
        ref={statsReveal.ref}
        className={`relative z-10 mx-4 sm:mx-14 rounded-2xl border border-white/8 bg-white/3 backdrop-blur-sm p-6 grid grid-cols-2 sm:grid-cols-4 gap-6 transition-all duration-700 ${statsReveal.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
      >
        {stats.map((s, i) => (
          <div
            key={s.label}
            className={`flex flex-col items-center gap-2 transition-all duration-600 ${statsReveal.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
            style={{ transitionDelay: `${i * 120}ms` }}
          >
            <div className="p-2 rounded-lg bg-white/5 border border-white/8 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-colors">
      ...
      <section className="relative z-10 px-4 sm:px-14 py-24">
        <div
          ref={featTitle.ref}
          className={`text-center mb-12 transition-all duration-700 ${featTitle.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
        >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {features.map((f, i) => (
            <RevealCard key={f.title} delay={i * 100}>
              <div
                className={`group relative h-full rounded-2xl border border-white/6 bg-gradient-to-br ${f.color} p-6 cursor-default transition-all duration-300 hover:border-white/15 hover:scale-[1.02]`}
                onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 8px 40px -8px ${f.glow}40`; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = "none"; }}
              >
                <div className="absolute top-3 right-3 h-1.5 w-1.5 rounded-full bg-white/15 group-hover:bg-white/50 transition-all duration-300" />
                ...
                <h3 className="font-semibold text-sm mb-1.5 text-white/90 group-hover:text-white transition-colors">{f.title}</h3>
                <p className="text-xs text-white/35 leading-relaxed group-hover:text-white/50 transition-colors">{f.desc}</p>
              </div>
            </RevealCard>
          ))}
        </div>
      ...
      <section className="relative z-10 px-4 sm:px-14 pb-24">
        <RevealCard>
          <div
            ref={ctaReveal.ref}
            className={`relative rounded-3xl border border-emerald-500/20 overflow-hidden p-10 sm:p-16 text-center transition-all duration-700 ${ctaReveal.visible ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
          ...
          >
            ...
            <div className="relative">
              <div className="inline-flex p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/25 mb-6 shadow-xl shadow-emerald-500/10 hover:scale-110 transition-transform duration-300">
